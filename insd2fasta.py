...
def parse_args():
	parser = argparse.ArgumentParser(description="Convert an INSDSeq NCBI file to FASTA format.")
	parser.add_argument("input_file", metavar="INSDXML_NCBI_FILE", type=str,
						help="Input INSDXML NCBI file (*.gbc)")
	parser.add_argument("-cds", action="store_true",
						help="Retrieves the CDS portion of all cDNA sequences (if CDS information is available), adds \"-\" to correct the ORF if necessary")
...
def insd2seqrecords(input_file, cds=False, sort=False):
	...
	for locus_id in sorted_loci:
		...
		seq_records.append(SeqRecord(Seq(locus[locus_id]['seq']), id="|".join(fasta_name), description=""))
